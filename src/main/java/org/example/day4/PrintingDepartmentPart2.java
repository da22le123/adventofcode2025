...
@SuppressWarnings("D")
public class PrintingDepartmentPart2 extends Puzzle {
    public PrintingDepartmentPart2(AocInput input) {
        super(input);
    }
    ...
    public Object solve() {
        ...
        while (removed == -1 || removed > 0) {
            removed=0;
            for (int i = 0; i < matrix.length; i++) {
                for (int j = 0; j < matrix[i].length; j++) {
                    if (matrix[i][j] == '@' && canAccess(i, j, matrix)) {
                        removed++;
                        matrix[i][j] = 'X';
                    }
                }
